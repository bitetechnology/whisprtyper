...
- The clipped motion stage (`overflow: hidden`, `clamp(380px, 46vh, 480px)` tall on desktop) is full-bleed across the viewport while its title and CTA stay within the content gutter. Its responsive SVG path is one shallow, non-self-intersecting S with no hook or abrupt crest: it enters off-left, crosses the measured pill center, and exits off-right with restrained vertical travel; the runtime viewBox matches rendered CSS pixels so glyphs are never stretched.
...
- An original Corporate Memphis-inspired flat editorial upper-body avatar sits directly above the pill at a larger display scale (up to 210px tall on desktop, 178px at 390px; the pill seam sits at 55% of the stage and the avatar top keeps at least 4px of stage clearance). It is shown in right-facing profile with thick rounded black outlines, hand-drawn organic curves, playful proportions, minimal facial features, simple geometric shapes, and a limited lavender/mint/coral/skin-tone palette with no gradients or textures. One flat butter-pastel organic blob sits behind the bust as a static backdrop accent — no outline, gradient, texture, or motion. Only the avatar's mouth cavity, tongue, and lower lip move during the speaking loop; its head, eye, hair, neck, expressive upper-body pose, torso, and backdrop remain still.
...
- At 390px the hero fills the first viewport below the two-row nav (`min-height: calc(100svh - 107px)`), keeping the next section below the fold. The motion stage grows to roughly `343px` (`clamp(280px, 88vw, 352px)`), the larger avatar remains fully inside the stage and overlaps only the pill edge (the pill widens to 1.15× so the bust base stays masked), and the connected sentence/ribbon follows one restrained shallow S instead of an entry hook or steep opposing arcs.
...
- The speed section keeps the qualified heading **“Speak up to 5x faster than you type.”** and the adjacent 45/220 WPM comparison remains explicitly illustrative rather than measured WhisprTyper throughput. Its one shared path becomes a broad soft S in stacked layouts: a low horizontal Keyboard leg moves left-to-right, a contained right-hand bend sweeps into the Voice card, a wide middle return moves toward the left, and a low Voice leg exits right. The middle return and exit legs keep roughly 118px between centerlines inside a deep reserved bottom band, both below the Voice copy, so the thick lavender stroke reads as a clear S rather than a merged U outline. Mobile labels remain horizontal (`rotate(0)`) and fade near clipped endpoints and nearly out on the steep vertical connectors — the local tangent is still computed in stacked mode, but only to drive that fade.
